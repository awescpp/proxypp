...
#define BOOST_TEST_MODULE test_http_beast_request_adapter
...
namespace proxypp::http::adapter::test
{
  ...
  {
    http_::request_header<> request_header;
    detail::add_header_should_add_header_when_missing(request_header);
  }

  BOOST_AUTO_TEST_CASE(add_header_should_not_override_existing_header)
  {
    http_::request_header<> request_header;
    detail::add_header_should_not_override_existing_header(request_header);
  }

  BOOST_AUTO_TEST_CASE(set_header_should_add_header_when_missing)
  {
    http_::request_header<> request_header;
    detail::set_header_should_add_header_when_missing(request_header);
  }

  BOOST_AUTO_TEST_CASE(set_header_should_override_existing_header)
  {
    http_::request_header<> request_header;
    detail::set_header_should_override_existing_header(request_header);
  }

  BOOST_AUTO_TEST_CASE(replace_header_should_replace_existing_header)
  {
    http_::request_header<> request_header;
    detail::replace_header_should_replace_existing_header(request_header);
  }
  ...
  {
    http_::request_header<> request_header;
    detail::remove_header_should_remove_existing_header(request_header);
  }
}
