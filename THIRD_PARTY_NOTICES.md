...
The proxy++ project itself is licensed under the Apache License, Version 2.0. Third-party software remains licensed under its own license terms. This file is
provided for attribution and license-compliance purposes and does not replace the original license texts distributed by the upstream projects.
...
The following Node.js packages are used for development, formatting, commit workflow, TypeScript type checking, and end-to-end testing. They are development dependencies and are not intended to be distributed with proxy++ binary releases.
...
When creating a source or binary release of proxy++, include this file together with the proxy++ `LICENSE` and `NOTICE` files.

For binary releases, also include the exact upstream license texts for any third-party libraries that are actually distributed with the release package, such as static or dynamic libraries linked into or shipped with proxy++.
...
For Node.js development dependencies, the upstream license text can usually be found in the corresponding package directory under `node_modules`, if the dependency is redistributed.
